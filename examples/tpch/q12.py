...
def q(data_folder):
    ...
    bodo.barrier()
    t1 = time.time()
    sel = (lineitem.L_RECEIPTDATE < date2) & (lineitem.L_COMMITDATE < date2) & (lineitem.L_SHIPDATE < date2) & (
            lineitem.L_SHIPDATE < lineitem.L_COMMITDATE) & (lineitem.L_COMMITDATE < lineitem.L_RECEIPTDATE) & (
            lineitem.L_RECEIPTDATE >= date1) & ((lineitem.L_SHIPMODE == 'MAIL') | (lineitem.L_SHIPMODE == 'SHIP'))
    flineitem = lineitem[sel]
    jn = flineitem.merge(orders, left_on='L_ORDERKEY', right_on='O_ORDERKEY')
    def g1(x): return ((x == "1-URGENT") | (x == "2-HIGH")).sum()
    def g2(x): return ((x != "1-URGENT") & (x != "2-HIGH")).sum()
